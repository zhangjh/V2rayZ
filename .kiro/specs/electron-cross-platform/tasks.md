...
- [ ] 2.4 编写属性测试验证 IPC 往返一致性
  - **属性 1: IPC 通信往返一致性**
  - **验证: 需求 1.2**

- [ ] 3. 实现配置管理服务
- [ ] 3.1 实现 ConfigManager 核心逻辑
  - 实现配置文件读写
  - 实现配置验证逻辑
  - 实现默认配置生成
  - _需求: 4.1, 4.2, 4.3_
...
- [ ] 4.2 编写属性测试验证 VLESS 解析正确性
  - **属性 25: VLESS URL 解析正确性**
  - **验证: 需求 8.1**

- [ ] 4.3 编写属性测试验证 Trojan 解析正确性
...
- [ ] 5. 实现路由规则管理服务
- [ ] 5.1 实现 RoutingRuleManager 核心逻辑
  - 实现全局代理模式规则生成
  - 实现智能分流模式规则生成
  - 实现自定义域名规则集成
  - _需求: 9.1, 9.2, 9.3_
...
- [ ] 5.3 编写属性测试验证规则修改触发重启
  - **属性 30: 路由规则修改触发重启**
  - **验证: 需求 9.4**

- [ ] 6. 实现平台抽象层 - 系统代理
- [ ] 6.1 实现 Windows 系统代理管理
  - 实现注册表读写逻辑
  - 实现代理启用/禁用
  - 实现代理状态查询
  - _需求: 6.1_
...
- [ ] 6.6 编写属性测试验证代理状态查询准确性
  - **属性 20: 代理状态查询准确性**
  - **验证: 需求 6.5**

- [ ] 7. 实现平台抽象层 - 自启动管理
- [ ] 7.1 实现 Windows 自启动管理
  - 实现注册表启动项添加/删除
  - 实现自启动状态查询
  - _需求: 11.1_
...
- [ ] 7.4 编写属性测试验证自启动往返一致性
  - **属性 32: 自启动往返一致性**
  - **验证: 需求 11.3**

- [ ] 7.5 编写属性测试验证自启动状态查询准确性
  - **属性 33: 自启动状态查询准确性**
  - **验证: 需求 11.4**

- [ ] 8. 实现日志管理服务
- [ ] 8.1 实现 LogManager 核心逻辑
  - 实现日志记录和存储
  - 实现日志文件轮转
  - 实现日志查询
  - 实现日志级别过滤
...
- [ ] 8.3 编写属性测试验证日志查询返回最近条目
  - **属性 22: 日志查询返回最近条目**
  - **验证: 需求 7.3**

- [ ] 8.4 编写属性测试验证错误日志包含堆栈
  - **属性 23: 错误日志包含堆栈信息**
  - **验证: 需求 7.4**

- [ ] 8.5 编写属性测试验证日志级别动态调整
  - **属性 24: 日志级别动态调整**
  - **验证: 需求 7.5**
...
- [ ] 9.11 编写属性测试验证代理启动生成正确进程
  - **属性 4: 代理启动生成正确的进程**
  - **验证: 需求 2.2**

- [ ] 9.12 编写属性测试验证代理停止清理资源
  - **属性 5: 代理停止清理所有资源**
  - **验证: 需求 2.3**
...
- [ ] 11. 迁移 React 前端代码
- [ ] 11.1 复制现有 React 组件到 Electron 项目
...
- [ ] 11.3 编写属性测试验证 UI 交互触发正确操作
  - **属性 2: UI 交互触发正确的后端操作**
  - **验证: 需求 1.4**

- [ ] 11.4 配置 Vite 用于 Electron 渲染进程
  - 配置 base 路径
  - 配置构建输出目录
  - _需求: 1.1_
...
- [ ] 13.4 编写属性测试验证跨平台托盘菜单一致性
  - **属性 8: 跨平台托盘菜单一致性**
  - **验证: 需求 3.2**

- [ ] 13.5 编写属性测试验证托盘操作触发正确行为
  - **属性 31: 托盘操作触发正确行为**
  - **验证: 需求 10.3**

- [ ] 14. 准备跨平台资源文件
- [ ] 14.1 下载和准备 Windows 平台资源
  - 下载 Windows 版本 sing-box.exe
...
- [ ] 14.2 下载和准备 macOS Intel (x64) 平台资源
...
- [ ] 14.2.1 下载和准备 macOS Apple Silicon (arm64) 平台资源
...
- [ ] 14.5 编写属性测试验证平台特定二进制加载
  - **属性 11: 平台特定的二进制文件加载**
  - **验证: 需求 3.5**

- [ ] 15. 配置构建和打包
- [ ] 15.1 配置 electron-builder
...
- [ ] 15.2 编写属性测试验证打包产物包含必要文件
  - **属性 34: 打包产物包含必要文件**
  - **验证: 需求 12.4**

- [ ] 15.3 编写属性测试验证打包输出包含版本信息
  - **属性 35: 打包输出包含版本信息**
  - **验证: 需求 12.5**
...
- [ ] 18.2 在 macOS Intel (x64) 上测试完整流程
  - 测试应用启动
  - 测试添加服务器
  - 测试启动/停止代理
  - 测试系统代理设置
...
- [ ] 18.2.1 在 macOS Apple Silicon (arm64) 上测试完整流程
  - 测试应用启动
  - 测试添加服务器
  - 测试启动/停止代理
  - 测试系统代理设置
