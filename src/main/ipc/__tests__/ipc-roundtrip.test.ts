...
describe('属性测试: IPC 通信往返一致性', () => {
  ...
  test('属性 1.1: 简单数据类型往返一致性', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.oneof(fc.string(), fc.integer(), fc.float(), fc.boolean(), fc.constant(null)),
        async (value) => {
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (nestedObj) => {
          ...
          expect(response.data.server.config.settings).toEqual(nestedObj.server.config.settings);
        }
      ),
      { numRuns: 50 }
    );
  ...
  test('属性 1.7: 空值处理一致性', async () => {
    const emptyValues = [null, {}, [], '', 0, false];
